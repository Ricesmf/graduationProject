...
/**
 * 案例设计： Server：S Client：clientA， clientB，clientC Client端行为： 提示信息后续添加****** 1.
 * 输入任意字符S表示“登录”请求 2. S返回提示输入标识号 3. 输入标识号---S判断是否唯一，返回判断结果，重新输入标识号，直到标志号合法 4.
 * 任意一个客户端登录，提示所有客户端，连接的客户端个数和列表。 5.
 * 发送消息，clientA发送一个消息，clientB，clientC显示clientA发送的消息内容，同理 6. 成功了，clientA收到S发送的反馈。
 * 
 * @author SMF
 * @time 2018年1月22日
 */
public class SNew extends ServerSocket {
	 ...
	 */
	public SNew() throws IOException {
						...
						if (clientSet.contains(identity_card)) {
							...
							if (null != tl && tl.size() > 1) {
								for (HandlerThread t : tl) {
									t.sendMessage("当前在线客户端个数为 ：" + clientSet.size());
									t.sendMessage("在线客户端为 ：" + clientSet.toString());
								}
							} else {
